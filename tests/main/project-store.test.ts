...
describe("ProjectStore", () => {
  ...
  beforeEach(() => {
    ...
    mockStore.set.mockImplementation((key: string, value: unknown) => {
      if (key === "projects") storedProjects = value as Project[];
      if (key === "activeProjectId") storedActiveProjectId = value as string | null;
      if (key === "worktreeCounter") storedWorktreeCounter = value as number;
    });
