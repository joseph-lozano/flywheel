...
- **Latency note:** `ptyManager.checkTitles()` polls every `TITLE_CHECK_INTERVAL` flush cycles (configurable at `pty-manager.ts:18`). Change from 60 (≈1s) to **30 (≈0.5s)** for snappier busy animation response. The cost is one `pty.process` syscall per terminal per check — at 0.5s this is ~2 calls/sec/terminal, negligible.
